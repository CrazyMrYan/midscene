...
1. Confirm the **feature name** and **target URL** with the user
2. Create `e2e/<feature>.test.ts` (kebab-case filename)
3. Scaffold using the template from [test-template.md](./test-template.md)
4. Fill in `describe` name, `it` blocks, and Midscene API calls per user requirements
5. Run and verify:
   ```bash
   npx vitest run e2e/<feature>.test.ts
   ```
...
3. Reuse the existing `ctx` — do NOT create a second `TestContext`
...
2. Remove the file: `e2e/<feature>.test.ts`
...
npx vitest run e2e/<feature>.test.ts   # Run a specific test
...
- **FAIL**: Check the error message — common causes:
  ...
  - Timeout → page may not have loaded, add `waitForLoadState` or increase timeout
  - Assertion failed → verify the expected condition matches actual page state
- **Timeout**: Default test timeout is 180s. For slow pages, add explicit waits or use `aiWaitFor`
